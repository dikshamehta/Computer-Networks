    ...
    if (in_headers) {
        ...
        for (int i=0; i<4; i++) {
            if (strcmp(tests[2].results[i], "UNTESTED") == 0)
                tests[2].results[i] = "FAIL - Header was not found in response";
        }
    }
...
void t__p2_tests(void) {
    ...
    for (int i=0; i<4; i++)
        assert(strcmp(tests[1].results[i], "UNTESTED") != 0, tests[2].results[i], "FAIL - No headers read, could not perform test");
